...
```
기본적인 MySQL DB 설정입니다. 추가적인 부분이라면 slave-list를 적어준 부분이고 이는 자동 설정이 아니며, 코드상에 사용할 값들입니다.
...
@AllArgsConstructor
@Builder
@Table(name = "member") // 작성해주지 않으면 안돌아감
public class Member {
...
```
주의해야할 점이 여기서는 Table 애노테이션으로 반드시 테이블 명을 명시해주어야 합니다.  
<Br>
...
```
아직 스프링에 대해 깊은 지식이 없어서 왜인지는 모르겠으나 yml에 ddl 설정을 추가하여도 테이블을 생성해주지 않아 위와 같이 직접 테이블을 생성해주어야 했습니다.
